...
import { getDocSection, DOC_NAV } from '@/lib/docs-content'
...
function renderContent(md: string): string {
  return md
    ...
    .replace(/```(\w+)?\n([\s\S]*?)```/g, (_, lang, code) => {
      ...
      return `<pre class="doc-code" data-lang="${lang ?? ''}">${escaped}</pre>`
    })
    // Tables
    .replace(/^\|(.+)\|\s*\n\|[-| :]+\|\s*\n((?:\|.+\|\s*\n?)+)/gm, (_, header, rows) => {
  ...
  return (
    <>
      ...
      <div className="mb-10 pb-8 border-b" style={{ borderColor: 'var(--border)' }}>
        <p className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: '#ff3d00', letterSpacing: '0.1em' }}>
          Documentation
        </p>
        <h1 className="text-3xl font-bold mb-3" style={{ letterSpacing: '-0.025em' }}>{section.title}</h1>
        <p className="text-base" style={{ color: 'var(--muted-text)' }}>{section.description}</p>
      </div>

      {/* Content */}
      <style>{`
        .doc-h2 { font-size: 1.375rem; font-weight: 700; letter-spacing: -0.02em; margin: 2.5rem 0 1rem; }
        .doc-h3 { font-size: 1.1rem; font-weight: 600; letter-spacing: -0.015em; margin: 2rem 0 0.75rem; }
        ...
        .doc-table td code, .doc-table th code { font-family: "JetBrains Mono", monospace; background: var(--surface-alt); padding: 0.1em 0.35em; border-radius: 0.25rem; font-size: 0.8rem; }
      `}</style>

      <div dangerouslySetInnerHTML={{ __html: html }} />

      {/* Prev/Next navigation */}
      <div className="flex items-center justify-between mt-14 pt-8 border-t" style={{ borderColor: 'var(--border)' }}>
        {prev ? (
          <Link href={`/docs/${prev.slug}`} className="group flex flex-col" style={{ color: 'var(--muted-text)' }}>
            <span className="text-xs uppercase tracking-wider mb-1" style={{ letterSpacing: '0.08em' }}>← Previous</span>
            <span className="text-sm font-semibold group-hover:opacity-70 transition-opacity" style={{ color: 'var(--foreground)' }}>{prev.title}</span>
          </Link>
        ) : <div />}
        {next ? (
          <Link href={`/docs/${next.slug}`} className="group flex flex-col text-right" style={{ color: 'var(--muted-text)' }}>
            <span className="text-xs uppercase tracking-wider mb-1" style={{ letterSpacing: '0.08em' }}>Next →</span>
            <span className="text-sm font-semibold group-hover:opacity-70 transition-opacity" style={{ color: 'var(--foreground)' }}>{next.title}</span>
          </Link>
        ) : <div />}
      </div>
    </>
